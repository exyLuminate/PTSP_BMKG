...
// Tambahkan parameter adminNote agar fungsi bisa membaca catatan
const getProgressInfo = (status, adminNote) => {
    switch (status) {
        case 'on_process': 
            // Ubah 'note' menjadi 'adminNote' sesuai parameter
            if (adminNote === 'User meminta kirim ulang kode billing.') {
        ...
        case 'invalid': return { step: 2, label: 'Billing Kadaluwarsa' };
...
// Update pemanggilan fungsinya di bawah (tambahkan argumen kedua)
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-6">
                    <div className="bg-white w-full max-w-md rounded-[3rem] p-12 shadow-2xl text-center">
                        ...
                        <form onSubmit={handleVerify} className="space-y-4">
                            ...
                            <button disabled={processing} className="w-full bg-blue-600 text-white font-black py-5 rounded-2xl uppercase text-[10px] tracking-widest shadow-lg">Buka Detail</button>
                        </form>
                    </div>
                </div>
            )}
            ...
            <div className={`max-w-4xl mx-auto transition-all duration-700 ${isVerified ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                ...
                <div className="bg-white rounded-[3.5rem] shadow-2xl border border-slate-100 overflow-hidden">
                    ...
                    <div className="p-12 border-b border-slate-50 bg-gradient-to-b from-slate-50/50 text-center">
                        ...
                        <div className={`text-4xl font-black uppercase tracking-tighter mb-8 leading-none ${request_data.status === 'rejected' || request_data.status === 'invalid' || request_data.status === 'expired' ? 'text-red-600' : 'text-slate-900'}`}>
                            {info.label}
                        </div>
                    ...
                    <div className="p-12 grid grid-cols-1 md:grid-cols-2 gap-12">
                        ...
                        <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-xl">
                            ...
                            {/* ALERT REVISI (Muncul hanya jika ada catatan admin saat menunggu bayar) */}
                            {request_data.status === 'waiting_payment' && request_data.admin_note && request_data.admin_note !== 'User meminta kirim ulang kode billing.' && (
                                <div className="bg-amber-500/10 border-2 border-amber-500/50 p-6 rounded-[2rem] mb-8 flex gap-5 items-start animate-bounce-subtle">
                                    ...
                                    <div className="flex-1">
                                        <h5 className="text-[10px] font-black text-amber-500 uppercase tracking-[0.2em] mb-1">Instruksi Perbaikan Admin</h5>
                                        <p className="text-sm font-bold text-amber-100 italic leading-relaxed">
                                            "{request_data.admin_note}"
                                        </p>
                                        <p className="text-[9px] text-amber-500/60 mt-2 font-bold uppercase tracking-widest leading-none">
                                            Silakan unggah kembali dokumen yang sesuai di bawah ini
                                        </p>
                                    </div>
                                </div>
                            )}
                            ...
                            {request_data.status === 'rejected' && (
                                <div className="space-y-6">
                                    <div className="bg-red-500/10 border border-red-500/20 p-6 rounded-2xl">
                                        ...
                                        <p className="text-sm font-medium italic text-red-50 leading-relaxed">"{request_data.admin_note || 'Berkas kurang lengkap.'}"</p>
                                    </div>
                                    <p className="text-[10px] text-slate-400 uppercase tracking-widest leading-loose italic">Silakan hubungi Admin PTSP untuk perbaikan data.</p>
                                </div>
                            )}

                            {/* CASE: INVALID (BILLING HABIS) */}
                            {request_data.status === 'invalid' && (
                                <div className="space-y-6">
                                    <div className="bg-amber-500/10 border border-amber-500/20 p-6 rounded-2xl text-amber-200">
                                        <p className="text-xs font-medium leading-relaxed">Masa berlaku billing (7 hari) telah habis. Silakan ajukan ulang kode billing baru.</p>
                                    </div>
                                    <Link 
                                        href={route('status.re_request', request_data.ticket_code)} 
                                        method="post" as="button"
                                        className="w-full bg-blue-600 py-4 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-700 transition shadow-lg shadow-blue-900/20"
                                    >
                                        Minta Billing Baru
                                    </Link>
                                </div>
                            ...
                            {request_data.status === 'waiting_payment' && (
                                <div className="space-y-6">
                                    <p className="text-sm font-medium opacity-80 leading-relaxed">Unduh Kode Billing (VA) dan upload bukti bayar di sini:</p>
                                    ...
                                    <form onSubmit={handleUploadProof} className="pt-6 border-t border-white/10 space-y-4">
                                        ...
                                        <button disabled={uploadForm.processing} className="w-full bg-emerald-500 py-4 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-600 transition shadow-lg">Kirim Bukti</button>
                                    </form>
                                </div>
                            )}

                            {/* CASE: PAID / DONE (DOWNLOAD HASIL) */}
                            {(request_data.status === 'paid' || request_data.status === 'done') && (
                                <div className="space-y-6 text-center">
                                    <p className="text-sm font-medium">Pembayaran Berhasil! {request_data.status === 'done' ? 'Data sudah diunduh.' : 'Silakan unduh data Anda:'}</p>
                                    <a 
                            ...
                            {request_data.status === 'expired' && (
                                <div className="py-10 text-center">
                                    ...
                                    <p className="text-[10px] text-slate-400 mt-2 italic leading-relaxed">Batas unduh 3 hari telah berakhir. Data telah dikunci demi keamanan.</p>
                                </div>
                            )}
                            ...
                            {request_data.status === 'on_process' && (
                                <p className="text-sm opacity-60 italic text-center py-10">
                                    {request_data.admin_note === 'User meminta kirim ulang kode billing.' 
                                        ? 'Permintaan billing baru sedang diproses Admin...' 
                            ...
                            {request_data.status === 'verifikasi_payment' && (
                                <p className="text-sm opacity-60 italic text-center py-10">
                                    Bukti bayar sedang divalidasi oleh Bendahara...
                                </p>
                            )}
                        </div>
                    </div>
...
// Komponen Pendukung (Icon & Preview)
        ...
        <div className="group">
            ...
            <div className="relative rounded-2xl overflow-hidden border border-white/10 bg-white/5 shadow-inner">
                ...
                <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <a href={streamUrl} target="_blank" className="bg-white text-slate-900 px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest">Buka Full ↗</a>
                </div>
            </div>
        </div>
    );
...
const IconDownload = () => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4 text-blue-600"><path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3" /></svg>;