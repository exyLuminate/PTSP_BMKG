...
export default function DetailStatus({ request_data }) {
    ...
    // Form untuk Verifikasi Password Akses
    const { data, setData, post, processing, errors } = useForm({
        ticket: request_data.ticket_code,
        password: '',
    });

    // Form Khusus untuk Upload Bukti Bayar
    ...
    const getProgressInfo = (status) => {
        switch (status) {
            case 'on_process': return { step: 1, label: 'Proses Verifikasi Berkas' };
            case 'waiting_payment': return { step: 2, label: 'Menunggu Pembayaran' };
            case 'verifikasi_payment': return { step: 2, label: 'Verifikasi Pembayaran' };
            case 'paid': return { step: 3, label: 'Pembayaran Lunas' };
    ...
    return (
        <div className="min-h-screen bg-slate-50 py-12 px-6 font-sans">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-6">
                    <div className="bg-white w-full max-w-md rounded-[3rem] p-12 shadow-2xl text-center">
                        ...
                        <form onSubmit={handleVerify} className="space-y-4">
                            ...
                            <button disabled={processing} className="w-full bg-blue-600 text-white font-black py-5 rounded-2xl uppercase text-[10px] tracking-widest">Buka Detail</button>
                        </form>
                    </div>
                </div>
            )}
            ...
            <div className={`max-w-4xl mx-auto transition-all duration-700 ${isVerified ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                {/* Header & Progress Bar sama seperti kode temanmu */}
    ...
    <div className="flex justify-between items-center max-w-md mx-auto relative mt-16">
        ...
        <div 
            ...
            style={{ width: `${info.step > 0 ? (info.step - 1) * 50 : 0}%` }}
        ></div>
        
        {[1, 2, 3].map((s) => (
            <div key={s} className="relative z-10 flex flex-col items-center">
                {/* Bulatan dengan Icon SVG */}
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-500 shadow-sm
                    ...
                    {s === 2 && (
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.051-.659-1.172-.879-1.172-2.303 0-3.182 1.172-.879 3.07-.879 4.242 0L15 8.818M3.75 21.75h16.5a2.25 2.25 0 0 0 2.25-2.25V4.5a2.25 2.25 0 0 0-2.25-2.25H3.75a2.25 2.25 0 0 0-2.25 2.25v15a2.25 2.25 0 0 0 2.25 2.25Z" />
                        </svg>
                    )}
                    {s === 3 && (
                ...
                <span className={`absolute -bottom-8 text-[9px] font-black uppercase tracking-widest whitespace-nowrap transition-colors duration-500
                    ...
                    <div className="p-12 grid grid-cols-1 md:grid-cols-2 gap-12">
                        ...
                        <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-xl">
                            ...
                            {/* CASE: Menunggu Pembayaran (Tampilkan PDF Billing) */}
                            {request_data.status === 'waiting_payment' && (
                                <div className="space-y-6">
                                    ...
                                    {/* Preview Billing PDF dari Admin */}
                                    ...
                                    <form onSubmit={handleUploadProof} className="pt-6 border-t border-white/10 space-y-4">
                                        ...
                                        <input 
                                            type="file" 
                                            onChange={e => uploadForm.setData('payment_proof', e.target.files[0])}
                                            className="w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-blue-600 file:text-white" 
                                            required 
                                        />
                            ...
                            {/* CASE: Selesai / Paid (Tampilkan Hasil Data) */}
...
// Komponen Pembantu untuk Preview PDF Privat
function FilePreview({ label, id, type, password }) {
    // Kita arahkan ke FileStreamController yang butuh password akses
